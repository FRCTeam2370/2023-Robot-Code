...
public class right_score_one_and_balance extends SequentialCommandGroup {
  /** Creates a new score_one. */
  public right_score_one_and_balance(frc.robot.subsystems.Swerve s_Swerve, sub_Elbow m_sub_Elbow, sub_Shoulder m_sub_Shoulder, sub_Gripper m_Sub_Gripper, Cube_Intake m_cube ) {
    ...
    new TrajectoryConfig(
      ...
      new TrajectoryConfig(
          ...
          TrajectoryGenerator.generateTrajectory(
        // Start at the origin facing the +X direction
        new Pose2d(-2.2, 0, new Rotation2d(180)),
        // Pass through these two interior waypoints, making an 's' curve path
      ...
      new TrajectoryConfig(
        ...
        // Pass through these two interior waypoints, making an 's' curve path
        List.of(new Translation2d(-.5, -3)),
        // End 3 meters straight ahead of where we started, facing forward
        new Pose2d(0, -3, new Rotation2d(180)),
        config4);
var thetaController =
...
addCommands(new CloseGripper(m_Sub_Gripper),new Top_goal(m_sub_Elbow, m_sub_Shoulder),  new InstantCommand(() -> s_Swerve.resetOdometry(exampleTrajectory.getInitialPose())),
  ...
  new WaitCommand(1), new stop_intake(m_cube), swerveControllerCommand3, swerveControllerCommand4, new auto_balence(s_Swerve, 0, 0), new InstantCommand(()-> Swerve.setoffset(0))
);
}
}